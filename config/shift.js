$(document).ready(function () {
    baosightRequire.requireFunct(['bxgrid', 'bxdiv', 'bxdialog', 'bxalert', 'bxcombobox'],
            function () {
                $("#queryarea").bxdiv();
                var gridOption = {
                    primaryRowKey: "shiftId",
                    caption: "班次信息维护",
                    colNames: ['班次Id', '班次编码', '班次名称', '描述'],
                    colModel: [{
                        name: 'shiftId',
                        index: 'shiftId',
                        width: 10,
                ...
                var option = {
                    queryParam: {},
                    dataPattern: "url",
                    url: "/config/shift.do?method=query",
                    showMsgOpt: {
                        showMsgId: "alertdiv"
                    },
    ...
    $("#inqu_status-shiftCode").keypress(function () {
        onKeyQuery();
    });
    $("#inqu_status-shiftName").keypress(function () {
        onKeyQuery();
    });
});
...
function insertRec() {
    ...
    AjaxCommunicator.ajaxRequest('/config/shift.do?method=insert', 'POST', paramJsonObj, callback);
}
...
function deleteOK() {
    ...
    AjaxCommunicator.ajaxRequest('/config/shift.do?method=delete', 'POST', paramJsonObj, callback);
}
...
function updateRec() {
    ...
    AjaxCommunicator.ajaxRequest('/config/shift.do?method=update', 'POST', paramJsonObj, callback);
}
